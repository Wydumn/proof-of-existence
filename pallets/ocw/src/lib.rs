...
use serde::{Deserialize, Deserializer};
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn offchain_worker(_block_number: T::BlockNumber) {
			...
			if let Ok(info) = Self::fetch_price_info() {
	...
	impl<T: Config> Pallet<T> {
		fn fetch_price_info() -> Result<DogePrice, http::Error> {
				// prepare for send request
				let deadline = sp_io::offchain::timestamp().add(Duration::from_millis(8_000));
				// query DOGE price
				let request =
						http::Request::get("https://api.binance.com/api/v3/avgPrice?symbol=DOGEUSDT");
				let pending = request
						.add_header("User-Agent", "Substrate-Offchain-Worker")
						.deadline(deadline).send().map_err(|_| http::Error::IoError)?;
				log::info!("OCW ==> price pending: {:?}", pending);
				let response = pending.try_wait(deadline).map_err(|_| http::Error::DeadlineReached)??;
				log::info!("OCW ==> price response: {:?}", response);
